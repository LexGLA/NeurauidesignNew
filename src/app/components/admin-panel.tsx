...
import {
  ...
  NotifItem, NotificationsContent, SearchContent,
  SubscribeContent, CustomerDetailContent, DealDetailContent,
  NewChatContent, CrmAddContent,
  FinDetailContent, FinAddContent,
  AgentSettingsContent, PersonnelSettingsContent,
  ProfileContent, HireAgentContent, CustomizeAgentContent,
  PermissionsContent, WorkflowContent, ToggleAgentContent,
  KpiSettingsContent, PlansManagementContent, AppSettingsContent,
  AboutContent, LogoutContent,
} from './admin-panel-modals';

// Re-export for backward compatibility
export { SettingsGroup, SettingsItem, SubscribeContent, CustomerDetailContent, ProfileContent, CustomizeAgentContent };

// Defined directly (not re-exported from modals) to avoid circular-dependency issues
// with meetings.tsx and secretary-screens.tsx, which import these AND are imported by this file.
...
export function FormSelect({ value, onChange, options }: { value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; options: { value: string; label: string }[] }) {
  return (
    <select value={value} onChange={onChange} className="w-full py-2.5 px-3.5 rounded-[10px] border border-[var(--aw-border)] text-[13px] text-[var(--aw-text-primary)] outline-none appearance-none" style={{ background: 'var(--aw-bg-input)' }}>
...
const TEAM_CONFIGS: Record<TeamKey, TeamConfig> = {
  ...
  secretary: {
    ...
    icon: 'fa-solid fa-calendar-check', bg: 'aw-bg-pink', gradient: 'linear-gradient(135deg, #EC4899, #BE185D)',
    agentId: 'secretary',
    navItems: [
  ...
  return (
    <div className="flex flex-col h-full items-center justify-center" style={{ background: 'var(--aw-bg-app)' }}>
      ...
      <div className="flex items-center justify-center gap-2 mb-6">
        <img src={neuraLogo} alt="Neura" className="h-10 w-auto object-contain" style={{ filter: 'var(--aw-logo-filter)' }} />
      ...
      {/* Role Switcher */}
      <div className="mt-10 w-full max-w-[320px] px-4">
        <RoleSwitcher />
      </div>
    </div>
  );
...
function AdminHeader() {
  const { company, setCompany, openModal, agentTeam, theme } = useApp();
  const isGlass = theme === 'glass';

  if (isGlass) {
    return (
      <header className="flex-shrink-0" style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)' }}>
        <div className="flex items-center px-4 md:px-6 pt-3 pb-1 gap-2">
          <button
            className="w-[34px] h-[34px] rounded-[10px] bg-transparent border-none cursor-pointer flex items-center justify-center"
            style={{ color: '#404040', fontSize: 16 }}
            onClick={() => openModal('تنظیمات', <MoreScreenModal />)}
          >
            <i className="fa-solid fa-gear" />
          </button>

          <div className="flex-1 flex flex-col items-center gap-[3px] md:hidden">
            <div className="flex items-center gap-1">
              <span style={{ fontFamily: "'Neogrey Medium', 'Neogrey', sans-serif", fontWeight: 600, fontSize: 15, color: '#565656', letterSpacing: '0.02em' }}>Neura</span>
              <img src={neuraLogo} alt="" className="h-[16px] w-auto object-contain" />
            </div>
            {agentTeam && (
              <span style={{ fontSize: 11, color: '#8f8f8f', fontWeight: 600 }}>{TEAM_CONFIGS[agentTeam]?.name}</span>
            )}
          </div>
          ...
          <button
            className="w-[34px] h-[34px] rounded-[10px] bg-transparent border-none cursor-pointer flex items-center justify-center"
            style={{ color: '#404040', fontSize: 16 }}
            onClick={() => openModal('اعلان‌ها', <NotificationsContent />)}
          >
  ...
  return (
    <header className="flex-shrink-0 border-b border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)' }}>
      <div className="flex items-center px-3 md:px-6 pt-2 mb-1.5 relative">
        {/* Right side: bell + company selector (mobile) */}
        <div className="flex gap-0.5 z-10">
          <HeaderBtn icon="fa-solid fa-bell" badge={3} onClick={() => openModal('اعلان‌ها', <NotificationsContent />)} />
          <div className="relative flex items-center gap-1 px-2 py-1 rounded-lg cursor-pointer border border-[var(--aw-border)] md:hidden" style={{ background: 'var(--aw-bg-card)' }}>
            <i className="fa-solid fa-building text-[10px] text-[var(--aw-primary)]" />
            <span className="text-[11px] text-[var(--aw-text-primary)] max-w-[60px] truncate" style={{ fontWeight: 600 }}>{COMPANIES[company]?.name.split(' ').slice(-1)[0]}</span>
        ...
        {/* Center: Logo (mobile) / Team name (desktop) */}
        <div className="flex-1 flex items-center justify-center gap-1.5 md:justify-start md:mr-4">
          ...
          {agentTeam && (
            <span className="hidden md:block text-[15px] text-[var(--aw-text-primary)]" style={{ fontWeight: 700 }}>
              {TEAM_CONFIGS[agentTeam]?.name || ''}
            </span>
          )}
        </div>

        {/* Left side: settings (mobile) | search + bell (desktop) */}
        <div className="flex gap-0.5 z-10">
          <div className="md:hidden">
            <HeaderBtn icon="fa-solid fa-sliders" onClick={() => openModal('تنظیمات', <MoreScreenModal />)} />
          </div>
      ...
      <div className="md:hidden"><RoleSwitcher /></div>
      <TeamIconBar />
    </header>
  );
}
...
export function RoleSwitcher() {
  ...
  if (isGlass) {
    return (
      <div
        className="flex gap-[3px] p-[3px] rounded-[9999px] border border-white/50"
        style={{ background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(20px)', boxShadow: '0 4px 4px rgba(123,98,252,0.2)' }}
      >
        {(['user', 'admin'] as const).map((r) => (
          <button
            ...
            style={role === r
              ? { background: '#7b62fc', color: '#fff', fontWeight: 700, padding: '7px 12px', fontSize: 12, boxShadow: 'inset 2px 2px 1px rgba(255,255,255,0.45), 2px 4px 4px rgba(0,0,0,0.25)' }
              : { background: 'transparent', color: '#8f8f8f', fontWeight: 500, padding: '7px 12px', fontSize: 12 }}
    ...
    <div className="flex gap-1 p-1 mx-4 mb-1.5 rounded-xl" style={{ background: 'var(--aw-bg-card)' }}>
      {(['admin', 'user'] as const).map((r) => (
        <button
          ...
          style={role === r ? { background: 'var(--aw-primary)', fontWeight: 600, boxShadow: '0 2px 10px rgba(155,89,182,0.3)' } : { fontWeight: 600 }}
          onClick={() => { setRole(r); if (r === 'admin') setBriefingSeen(true); }}
        >
          {r === 'admin' ? 'پنل مدیریت' : 'پنل شخصی'}
        </button>
...
function AdminSidebar({ expanded, onToggle }: { expanded: boolean; onToggle: () => void }) {
  ...
  return (
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              style={isActive ? { background: 'var(--aw-primary)', fontWeight: 600, boxShadow: '0 2px 10px rgba(155,89,182,0.3)' } : { fontWeight: 500 }}
              onClick={() => setAdminScreen(item.id)}
            >
      ...
      {expanded && (
        <div className="px-3 pb-3"><RoleSwitcher /></div>
      )}
    </aside>
  );
}
  ...
  return (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pb-2.5 flex-shrink-0">
        <div className="flex gap-1 p-1 rounded-[10px] overflow-x-auto aw-scroll" style={{ background: 'var(--aw-bg-card)' }}>
          {allTabs.map(t => {
            ...
            return (
              <div
                key={t.id}
                className={`flex-shrink-0 flex items-center rounded-lg transition-all ${isActive ? 'text-white' : 'bg-transparent text-[var(--aw-text-muted)]'}`}
                style={isActive ? { background: 'var(--aw-primary)' } : undefined}
              >
                <button
                  className="px-3 py-2 text-center border-none bg-transparent text-inherit text-[12px] cursor-pointer whitespace-nowrap"
            ...
            <div className="md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-1">
              {filteredTeamSamples.map(s => (
                <div key={s.id}
                  ...
                  <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center text-white flex-shrink-0 ${s.bg}`} style={{ fontWeight: 700, fontSize: 18 }}>
                    {s.init}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <button
                    className="w-9 h-9 rounded-[10px] border-none text-white cursor-pointer flex items-center justify-center text-[12px] flex-shrink-0"
                    style={{ background: 'var(--aw-secondary)' }}
                    onClick={(e) => { e.stopPropagation(); startCall(s.name, s.tag, s.bg, s.init, ''); }}
                  >
...
function AgentChatItem({ agent: a }: { agent: Agent }) {
  const { openChat, openModal } = useApp();

  if (a.locked) {
    return (
      <div
        className="flex items-center gap-3 p-3 rounded-[14px] cursor-pointer transition-all mb-1 opacity-60"
...
function PersonnelChatItem({ person: p }: { person: any }) {
  ...
  return (
    ...
    >
      <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center text-white flex-shrink-0 relative ${p.bg}`} style={{ fontWeight: 700, fontSize: 18 }}>
        {p.init}
  ...
  return (
    <motion.div className="flex-1 overflow-y-auto p-4 pb-4 aw-scroll" variants={pageVariants} initial="initial" animate="animate" exit="exit">
      <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 mb-4" variants={staggerContainer} initial="initial" animate="animate">
        <motion.div variants={fadeInUp}><StatCard icon="fa-solid fa-robot" color="var(--aw-primary)" bg="rgba(155,89,182,0.15)" value={toFa(companyAgents.length)} label="عامل فعال" onClick={() => { setAdminScreen('chatsScreen'); setChatTab('agents'); }} /></motion.div>
        ...
        <div className="w-full h-[200px] mb-3" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={agentChartData} barGap={4} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              <Bar key="bar-pending-dash" dataKey="pending" name="معلق" fill="#B83D9E" radius={[6, 6, 0, 0]} barSize={18} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        {companyAgents.map(a => (
      ...
      <DashSection title="روند درآمد ماهانه" icon="fa-solid fa-chart-area" iconColor="var(--aw-secondary)" onTitleClick={() => { setAdminScreen('financeScreen'); setFinTab('overview'); }} actionLabel="ماژول مالی">
        <div className="w-full h-[180px]" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart id="admin-revenue-area" data={MONTHLY_REVENUE_DATA.slice(-6)} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              <defs>
...
function DealCard({ deal: d }: { deal: Deal }) {
  ...
  const stageStyle: Record<string, any> = {
    ...
    proposal: { background: 'rgba(155,89,182,0.2)', color: '#9B59B6' },
    ...
    <motion.div variants={pageVariants} initial="initial" animate="animate">
      ...
      <DashSection title="مقایسه درآمد و هزینه" icon="fa-solid fa-chart-line" iconColor="var(--aw-secondary)">
        <div className="w-full h-[220px]" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart id="admin-finance-area" data={MONTHLY_REVENUE_DATA} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
              <defs>
  ...
  return (
    <motion.div className="flex-1 overflow-y-auto p-4 pb-4 aw-scroll" variants={pageVariants} initial="initial" animate="animate" exit="exit">
        ...
        <div className="w-full h-[160px] mb-2" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={agentChartData} barGap={3} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              <Bar key="bar-pending-rpt" dataKey="pending" name="معلق" fill="#B83D9E" radius={[4, 4, 0, 0]} barSize={14} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </ReportCard>
      ...
      >
        <div className="w-full h-[160px] mb-2" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart id="admin-finance-line" data={MONTHLY_REVENUE_DATA.slice(-6)} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--aw-border-light)" />
              <XAxis dataKey="month" tick={{ fill: 'var(--aw-text-muted)', fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: 'var(--aw-text-muted)', fontSize: 9 }} axisLine={false} tickLine={false} width={25} />
              <Tooltip content={<ChartTooltip />} />
      ...
      >
        <div className="w-full h-[140px] mb-2" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart id="admin-weekly-bar" data={WEEKLY_ACTIVITY_DATA} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              <Bar key="bar-calls-rpt" dataKey="calls" name="تماس‌ها" fill="#9B59B6" radius={[4, 4, 0, 0]} barSize={12} />
            </BarChart>
          </ResponsiveContainer>
        </div>
  ...
  return (
    <div>
      <SettingsGroup title="مدیریت">
        <SettingsItem icon="fa-solid fa-robot" label="تنظیمات عامل‌ها" onClick={() => handleNav(() => openModal('تنظیمات عامل‌ها', <AgentSettingsContent />))} />
        <SettingsItem icon="fa-solid fa-user-tie" label="مدیریت پرسنل" onClick={() => handleNav(() => openModal('مدیریت پرسنل', <PersonnelSettingsContent />))} />
      ...
      <SettingsGroup title="عامل‌های هوشمند (AI)">
        <SettingsItem icon="fa-solid fa-user-plus" label="استخدام ایجنت" onClick={() => handleNav(() => openModal('استخدام ایجنت', <HireAgentContent />))} />
        <SettingsItem icon="fa-solid fa-shield-halved" label="پرمیشن و دسترسی" onClick={() => handleNav(() => openModal('پرمیشن و دسترسی', <PermissionsContent />))} />
        <SettingsItem icon="fa-solid fa-diagram-project" label="تنظیمات ورک‌فلو" onClick={() => handleNav(() => openModal('تنظیمات ورک‌فلو', <WorkflowContent />))} />
        <SettingsItem icon="fa-solid fa-toggle-on" label="فعال/غیرفعال‌سازی ایجنت" onClick={() => handleNav(() => openModal('فعال/غیرفعال‌سازی ایجنت', <ToggleAgentContent />))} />
        <SettingsItem icon="fa-solid fa-gauge-high" label="تنظیمات KPI" onClick={() => handleNav(() => openModal('تنظیمات KPI', <KpiSettingsContent />))} />
        <SettingsItem icon="fa-solid fa-crown" label="مدیریت پلن‌ها" onClick={() => handleNav(() => openModal('مدیریت پلن‌ها', <PlansManagementContent />))} />
      </SettingsGroup>

      <SettingsGroup title="ارتباطات">
        <SettingsItem icon="fa-solid fa-phone-alt" label="تماس‌ها" onClick={() => { closeModal(); openUnifiedCall(); }} />
        <SettingsItem icon="fa-solid fa-bell" label="اعلان‌ها" onClick={() => handleNav(() => openModal('اعلان‌ها', <NotificationsContent />))} />
      </SettingsGroup>

      <SettingsGroup title="حساب کاربری">
        <SettingsItem icon="fa-solid fa-user-circle" label="پروفایل من" onClick={() => handleNav(() => openModal('پروفایل من', <ProfileContent />))} />
        <SettingsItem icon="fa-solid fa-cog" label="ت��ظیمات برنامه" onClick={() => handleNav(() => openModal('تنظیمات برنامه', <AppSettingsContent />))} />
        <SettingsItem icon="fa-solid fa-question-circle" label="درباره Neura" onClick={() => handleNav(() => openModal('درباره Neura', <AboutContent />))} />
        <SettingsItem icon="fa-solid fa-sign-out-alt" label="خروج" danger onClick={() => handleNav(() => openModal('خروج', <LogoutContent />))} />
      </SettingsGroup>
    </div>
  );
}
...
const CALENDAR_EVENTS: CalendarEvent[] = [
  ...
  { id: 'ce6', day: 14, title: 'وبینار آموزشی', color: '#EC4899', time: '۱۵:۰۰' },
        ...
        {activeTab === 'tasks' && (
          <div className="grid grid-cols-3 gap-2 mb-3">
            <div className="rounded-[12px] p-3 text-center border border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-card)' }}>
              <div className="text-lg" style={{ fontWeight: 800, color: '#9B59B6' }}>{toFa(todoCount)}</div>
...
export default function AdminPanel() {
  ...
  return (
   <MarketingProvider>
    <div className="flex h-full min-h-0 relative" style={{ direction: 'rtl' }}>
