...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // Screens
  const [adminScreen, setAdminScreen] = useState<AdminScreen>('chatsScreen');
  const [euScreen, setEuScreen] = useState<EuScreen>('euHomeScreen');
  ...
  const [theme, setTheme] = useState<ThemeMode>(() => {
    try {
      ...
      if (saved === 'dark' || saved === 'light' || saved === 'bw' || saved === 'glass') return saved;
    } catch {}
    return 'glass';
  });
  ...
  // Modal
  const openModalFn = useCallback((title: string, content: React.ReactNode) => {
    setModal({ open: true, title, content });
  }, []);
  const closeModalFn = useCallback(() => setModal({ open: false, title: '', content: null }), []);
