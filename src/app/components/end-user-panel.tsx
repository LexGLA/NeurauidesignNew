...
import { RoleSwitcher, SubscribeContent, ProfileContent, FormGroup, FormInput, SettingsGroup, SettingsItem, CustomizeAgentContent } from './admin-panel';
...
function EuHeader() {
  const { openModal, euScreen, setEuScreen, theme } = useApp();
  ...
  if (isGlass) {
    return (
      <header className="flex-shrink-0" style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)' }}>
        {/* Top icon row — matches Figma Frame24: icons on left, Neura logo on right */}
        <div className="flex items-center relative md:hidden px-[16px] pt-[26px] pb-[8px]">
          {/* LEFT side: 3 icons (setting, notification, search) */}
          {agentMeta ? (
            <button
            ...
            <div className="flex items-center gap-[16px]">
              <button
                ...
                style={{ color: '#404040', fontSize: 16 }}
                onClick={() => openModal('تنظیمات هوشمند', <EuSettingsContent />)}
              >
              ...
              >
                <i className="fa-solid fa-bell" style={{ fontSize: 16 }} />
                <span className="absolute -top-1 -right-1 w-[14px] h-[14px] rounded-full flex items-center justify-center text-white" style={{ background: '#ef4444', fontSize: 9, fontWeight: 700 }}>۳</span>
              </button>
              <button
                className="w-[20px] h-[20px] bg-transparent border-none cursor-pointer flex items-center justify-center p-0"
                style={{ color: '#404040', fontSize: 16 }}
              ...
              >
                <i className="fa-solid fa-magnifying-glass" style={{ fontSize: 16 }} />
              </button>
            </div>
          )}

          {/* RIGHT side (RTL spacer + Neura logo) */}
          <div className="flex-1 flex items-center justify-end gap-[3px]">
            {agentMeta ? (
              <>
                <i className={`${agentMeta.icon} text-[12px]`} style={{ color: agentMeta.color }} />
                <span style={{ fontSize: 13, fontWeight: 700, color: '#565656' }}>{agentMeta.title}</span>
              </>
            ) : (
              <>
                <span style={{ fontFamily: "'Neogrey Medium', 'Neogrey', sans-serif", fontWeight: 500, fontSize: 16, color: '#565656' }}>Neura</span>
        ...
        <div className="hidden md:flex items-center px-6 pt-3 pb-2 gap-2">
          ...
          <button className="w-[34px] h-[34px] rounded-[10px] bg-transparent border-none cursor-pointer text-[#404040] text-[16px] flex items-center justify-center"
            onClick={() => openModal('تنظیمات هوشمند', <EuSettingsContent />)}>
            <i className="fa-solid fa-gear" />
          </button>
        </div>

        {/* Avatar row — large, centered, matches Figma */}
        {!agentMeta && (
          <div className="md:hidden flex flex-col items-center pb-0">
            <div className="w-[400px] h-[400px] rounded-full overflow-hidden mx-[0px] my-[-71px]">
              <img src={copilotAvatar} alt="پروفایل" className="w-full h-full object-cover mx-[0px] my-[-18px]" />
            </div>
          </div>
        )}

        {/* Mobile: role switcher pill — only on non-agent screens */}
        {!agentMeta && (
          <div className="md:hidden px-[16px] pt-[0px] pb-[8px]" style={{ marginTop: -1 }}>
            <RoleSwitcher />
          </div>
        )}
      </header>
  ...
  return (
    <header className="flex-shrink-0 border-b border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)' }}>
      ...
      {!agentMeta && <div className="md:hidden"><RoleSwitcher /></div>}
    </header>
  );
}
    ...
    >
      {EU_NAV_ITEMS.map((item) => {
        ...
        return (
          ...
          >
            {isActive && (
              <span className="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-[3px] rounded-full" style={{ background: 'var(--aw-eu-primary)' }} />
            )}
            <i className={`${item.icon} text-[18px]`} />
            <span>{item.label}</span>
            {badge != null && badge > 0 && (
              <span className="absolute top-0.5 right-1/4 w-[16px] h-[16px] rounded-full flex items-center justify-center text-white text-[9px]" style={{ background: 'var(--aw-danger)', fontWeight: 700 }}>
                {toFa(badge)}
              </span>
            )}
          </button>
...
// SWIPE TO CALL
// ========================
function SwipeToCall({ onCall, children }: { onCall: () => void; children: React.ReactNode }) {
  ...
  const begin = (x: number) => { startX.current = x; triggered.current = false; setDragging(true); };
  const move = (x: number) => {
    if (startX.current == null) return;
    const d = Math.max(0, x - startX.current);
    setDx(Math.min(d, MAX));
  };
  const end = () => {
    if (startX.current == null) return;
    const shouldFire = dx >= THRESHOLD && !triggered.current;
    if (shouldFire) { triggered.current = true; onCall(); }
    startX.current = null;
    setDragging(false);
    setDx(0);
  };
...
function EuChatListScreen() {
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-4 aw-scroll">
      {/* Tab bar */}
      <div className="flex gap-1 p-1 mx-3 mt-3 mb-1 rounded-xl" style={{ background: 'var(--aw-bg-hover)' }}>
        <button
          className="flex-1 py-2.5 rounded-lg border-none cursor-pointer text-[13px] flex items-center justify-center gap-1.5 transition-all"
          style={{
            background: chatListTab === 'interactions' ? 'var(--aw-eu-primary)' : 'transparent',
            color: chatListTab === 'interactions' ? '#fff' : 'var(--aw-text-secondary)',
            fontWeight: chatListTab === 'interactions' ? 700 : 500,
          }}
          onClick={() => setChatListTab('interactions')}
        >
          <i className="fa-solid fa-comments text-[12px]" /> تعاملات من
        </button>
        <button
          className="flex-1 py-2.5 rounded-lg border-none cursor-pointer text-[13px] flex items-center justify-center gap-1.5 transition-all"
          style={{
            background: chatListTab === 'agents' ? 'var(--aw-eu-primary)' : 'transparent',
            color: chatListTab === 'agents' ? '#fff' : 'var(--aw-text-secondary)',
            fontWeight: chatListTab === 'agents' ? 700 : 500,
          }}
          onClick={() => setChatListTab('agents')}
        >
          <i className="fa-solid fa-robot text-[12px]" /> عامل هوشمند
        </button>
      </div>
              ...
              return items.map(item => {
                ...
                return (
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-[14px] cursor-pointer border border-transparent hover:bg-[var(--aw-bg-card-hover)] active:scale-[0.98]"
                    ...
                    <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center text-white flex-shrink-0 relative ${u.bg}`} style={{ fontWeight: 700, fontSize: 18 }}>
                      {u.init}
                      {u.online && (
                        <span className="absolute bottom-0.5 left-0.5 w-[13px] h-[13px] rounded-full border-2 border-[var(--aw-bg-app)]" style={{ background: 'var(--aw-online)' }} />
                      )}
...
function EuAgentDiscoveryScreen() {
  const { agents, openChat, startCall, openModal, setEuScreen, openUnifiedCall } = useApp();
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-4 aw-scroll">
      ...
      <div className="text-center py-6">
        <div className="w-20 h-20 rounded-full flex items-center justify-center text-white text-[32px] mx-auto mb-3 overflow-hidden" style={{ background: 'linear-gradient(135deg, var(--aw-eu-primary), var(--aw-eu-primary-dark))', fontWeight: 700 }}>
          {euProfile.avatarImage
            ? <img src={euProfile.avatarImage} alt={euProfile.name} className="w-full h-full object-cover" />
            : euProfile.avatar}
        </div>
        <div className="flex items-center justify-center gap-2">
          <div className="text-lg" style={{ fontWeight: 700 }}>{euProfile.name}</div>
          {euProfile.verified ? (
        ...
        {euProfile.username && <div className="text-[11px] text-[var(--aw-eu-primary)]" style={{ fontWeight: 600 }}>@{euProfile.username}</div>}
        <div className="text-[12px] text-[var(--aw-text-secondary)]">{euProfile.role}</div>
        {euProfile.email && <div className="text-[11px] text-[var(--aw-text-muted)] mt-1">{euProfile.email}</div>}
      </div>

      {/* Account Settings */}
      <div className="px-4">
        <div className="text-[12px] text-[var(--aw-text-muted)] mb-2 px-1" style={{ fontWeight: 700 }}>
          <i className="fa-solid fa-user-gear" /> حساب کاربری
        </div>
        <EuSettingsItem icon="fa-solid fa-edit" label="ویرایش پروفایل" onClick={() => openModal('پروفایل من', <ProfileContent />)} />
        <EuSettingsItem icon="fa-solid fa-wallet" label="مالی" onClick={() => openModal('بخش مالی', <EuFinanceContent />)} />
        <EuSettingsItem icon="fa-solid fa-shield-halved" label="تنظیمات امنیتی" onClick={() => openModal('تنظیمات امنیتی', <EuSecurityContent />)} />
        <EuSettingsItem icon="fa-solid fa-bell" label="اعلان‌ها" onClick={() => openModal('اعلان‌ها', <EuNotifications />)} />
        <EuSettingsItem icon="fa-solid fa-cog" label="تنظیمات" onClick={() => openModal('تنظیمات', <EuAppSettings />)} />
      </div>

      {/* Other Settings */}
      <div className="px-4 mt-4">
        <div className="text-[12px] text-[var(--aw-text-muted)] mb-2 px-1" style={{ fontWeight: 700 }}>
          <i className="fa-solid fa-ellipsis" /> سایر
        </div>
        <EuSettingsItem icon="fa-solid fa-headset" label="پشتیبانی" onClick={() => openModal('پشتیبانی', <EuSupportContent />)} />
        <EuSettingsItem icon="fa-solid fa-circle-question" label="سوالات متداول" onClick={() => openModal('سوالات متداول', <EuFaqContent />)} />
        <EuSettingsItem icon="fa-solid fa-file-contract" label="قوانین و حریم خصوصی" onClick={() => openModal('قوانین و حریم خصوصی', <EuLegalContent />)} />
        <EuSettingsItem icon="fa-solid fa-question-circle" label="راهنما" onClick={() => openModal('راهنما', <HelpContent />)} />
        <EuSettingsItem icon="fa-solid fa-sign-out-alt" label="خروج" danger onClick={() => openModal('خروج', <EuLogoutContent />)} />
      </div>
    </div>
  );
}
...
function EuSettingsItem({ icon, label, onClick, danger }: { icon: string; label: string; onClick?: () => void; danger?: boolean }) {
  return (
    <div className="flex items-center gap-3 p-3.5 rounded-xl mb-1 cursor-pointer border border-[var(--aw-border)] hover:border-[var(--aw-eu-primary)] transition-all" style={{ background: 'var(--aw-bg-card)' }} onClick={onClick}>
      <i className={`${icon} text-base w-5 text-center`} style={{ color: danger ? 'var(--aw-danger)' : 'var(--aw-eu-primary)' }} />
