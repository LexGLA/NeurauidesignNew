...
export default function ChatOverlay() {
  ...
  return (
    <>
    {/* Desktop backdrop */}
    <motion.div className="hidden md:block fixed inset-0 z-[99]" style={{ background: 'rgba(0,0,0,0.35)' }} onClick={closeChat} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} />

    {/* Mobile: tap the area above popup to close */}
    <div
      className="md:hidden fixed z-[99]"
      style={{ top: 0, left: 0, right: 0, height: headerBottom }}
      onClick={closeChat}
    />
    ...
    <motion.div
      ...
      style={window.innerWidth >= 768 ? {
        ...
        width: 500,
        borderRadius: 0,
        background: 'var(--aw-bg-app)',
        backdropFilter: 'none',
        boxShadow: '2px 0 20px rgba(0,0,0,0.15)',
        borderRight: '1px solid var(--aw-border)',
      } : {
        /* Mobile: panel starting exactly below the header */
        top: headerBottom,
    ...
    >
      {/* Drag handle — only on desktop where it's a sliding panel */}
      ...
      {/* Header — agent info card matching Figma */}
      <div
      ...
      >
        ...
        {/* Avatar circle */}
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0 relative overflow-hidden ${headerBg} ${chat.type === 'group' ? 'cursor-pointer' : ''}`}
        ...
        <div
          ...
          onClick={() => { if (chat.type === 'group' && chat.id) openModal('تنظیمات گروه', <GroupSettingsContent groupId={chat.id} />); }}
        >
          <div className="text-[14px] truncate" style={{ fontWeight: 700, color: '#333', fontFamily: "'Kamand', sans-serif" }}>
            {headerName}
          </div>
        ...
        <div className="flex gap-1 flex-shrink-0">
          ...
          <button
            className="w-8 h-8 rounded-full border-none cursor-pointer flex items-center justify-center"
            style={historyOpen ? { background: 'rgba(123,98,252,0.18)', color: '#7b62fc', fontSize: 13 } : { background: 'rgba(0,0,0,0.06)', color: '#565656', fontSize: 13 }}
          ...
          {showTopicsToggle && (
            <button
              className="w-8 h-8 rounded-full border-none cursor-pointer flex items-center justify-center"
              style={chat.topicsOpen ? { background: 'rgba(123,98,252,0.18)', color: '#7b62fc', fontSize: 13 } : { background: 'rgba(0,0,0,0.06)', color: '#565656', fontSize: 13 }}
      ...
      <div className="flex-1 overflow-hidden flex relative">
        ...
        {/* Messages */}
        <div ref={msgsRef} className="flex-1 overflow-y-auto px-3 pt-2 pb-3 flex flex-col gap-2 aw-scroll">
          {messages.map(m => (
            <div key={m.id} className={`max-w-[78%] px-3.5 py-2.5 rounded-[18px] text-[13px] relative break-words ${
              ...
              color: m.sent ? '#fff' : '#333',
              lineHeight: 1.6,
              boxShadow: m.sent
                ? '0 2px 8px rgba(123,98,252,0.25)'
                : '0 1px 4px rgba(0,0,0,0.08)',
              backdropFilter: m.sent ? 'none' : 'blur(8px)',
              border: m.sent ? 'none' : '0.5px solid rgba(255,255,255,0.8)',
          ...
          {/* Typing indicator */}
          {isTyping && (
            <div className="self-end px-4 py-3 rounded-[18px]" style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)', border: '0.5px solid rgba(255,255,255,0.8)' }}>
