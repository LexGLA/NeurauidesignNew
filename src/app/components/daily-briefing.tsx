...
export default function DailyBriefingScreen({ onDismiss }: { onDismiss: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full relative overflow-hidden" style={{ background: 'var(--aw-bg-app)' }}>
      {/* ===== AVATAR + SPEECH BUBBLE (fixed top) ===== */}
      <div className="flex-shrink-0">
        <motion.div
          className="flex flex-col items-center pt-5 pb-3 px-4 relative"
          animate={isDismissing ? { opacity: 0, scale: 0.8, y: 20 } : { opacity: 1, scale: 1, y: 0 }}
        ...
        >
          ...
          >
            ...
            <div
              className="w-[88px] h-[88px] rounded-full flex items-center justify-center text-white text-[38px] relative z-10"
              style={{
                background: 'linear-gradient(135deg, #EC4899, #BE185D)',
                boxShadow: '0 6px 28px rgba(236,72,153,0.3)',
                fontWeight: 700,
              }}
            >
              {agentInit}
            ...
            <div
              className="absolute bottom-0.5 right-0.5 w-4.5 h-4.5 rounded-full z-20 flex items-center justify-center"
          ...
          <motion.div
            className="flex items-center justify-center gap-[2.5px] h-6 mb-2"
            initial={{ opacity: 0 }}
            animate={{ opacity: isSpeaking ? 1 : 0.25 }}
          ...
          >
            {waveBars.map((_, i) => (
              <motion.div
                ...
                style={{ background: 'linear-gradient(to top, #EC4899, #BE185D)' }}
                animate={isSpeaking ? {
                  height: [3 + Math.random() * 3, 6 + Math.random() * 14, 3 + Math.random() * 3],
                } : { height: 3 }}
                transition={{
          ...
          >
            ...
            {/* Skip speech button (inside bubble) */}
            {!allDone && (
              <button
                className="absolute top-2 left-2 px-2 py-1 rounded-md border border-[var(--aw-border)] bg-transparent text-[10px] text-[var(--aw-text-muted)] cursor-pointer hover:text-[var(--aw-text-primary)] hover:border-[var(--aw-primary)] transition-colors z-20"
                style={{ fontWeight: 500 }}
                onClick={handleSkip}
              >
                رد شدن <i className="fa-solid fa-forward text-[8px] mr-0.5" />
              </button>
            )}
          </motion.div>
        </motion.div>
          ...
          <BriefingSection visible={showTasks} id="tasks">
            <div className="rounded-2xl p-3.5 border border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-card)' }}>
              <div className="flex items-center gap-2 mb-2.5">
                <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(155,89,182,0.15)' }}>
                  <i className="fa-solid fa-list-check text-[12px]" style={{ color: '#9B59B6' }} />
                </div>
              ...
              <div className="rounded-2xl p-3.5 border border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-card)' }}>
                <div className="flex items-center justify-between mb-2.5">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(155,89,182,0.15)' }}>
                      <i className="fa-solid fa-chart-bar text-[12px]" style={{ color: '#9B59B6' }} />
                    </div>
      ...
      <div className="flex-shrink-0 px-4 pb-3 pt-2">
        <BriefingSection visible={showCTA} id="cta">
          <button
      ...
      {/* Morphing avatar — flies to FAB position during dismiss */}
      <AnimatePresence>
        {isDismissing && avatarRect && (
          <motion.div
            className="fixed z-[999] rounded-full flex items-center justify-center text-white pointer-events-none"
            style={{
              background: 'linear-gradient(135deg, #EC4899, #BE185D)',
              fontWeight: 700,
            }}
            ...
            animate={{
              ...
              height: 48,
              fontSize: 18,
              top: typeof window !== 'undefined' ? window.innerHeight - 64 - 48 : 'calc(100vh - 112px)',
              left: 16,
          ...
          >
            ...
            >
              <i className="fa-solid fa-microphone text-[8px]" style={{ color: '#EC4899' }} />
            </motion.span>
          </motion.div>
        )}
      </AnimatePresence>
